...
class VehicleCirculationPlanner(QObject):
    ...
    def create_circulation_plan(self) -> bool:
    ...
    def validate_plans(self) -> bool:
        if len(self.plans) != 2:
            self.error_occurred.emit(ErrorMessages.PLAN_CREATION_FAILED.value)
            return False
        return True
    ...
    def create_updated_pivot_tables(self, merged_df):
        ...
        self.plans[0].timetable_data.timetable = df_direction_0.pivot(
        ...
        self.plans[1].timetable_data.timetable = df_direction_1.pivot(
            index=['sorted_date', 'sorted_day', 'stop_sequence', 'sorted_stop_name', 'sorted_stop_id'],
            columns=['sorted_start_time', 'sorted_trip_id', 'vehicle_number'],
            values='sorted_arrival_time').sort_index(
            axis=1).sort_index(
    ...
    def add_vehicle_number(self):
        self.plans[0].timetable_data.gtfs_table_data['vehicle_number'] = 0
        self.plans[1].timetable_data.gtfs_table_data['vehicle_number'] = 0
        self.plans[0].timetable_data.gtfs_table_data['direction'] = 0
        self.plans[1].timetable_data.gtfs_table_data['direction'] = 1
        self.plans[0].timetable_data.gtfs_table_data['first_vehicle_trip'] = ''
        self.plans[1].timetable_data.gtfs_table_data['first_vehicle_trip'] = ''
        merged_df = pd.concat(
            [self.plans[0].timetable_data.gtfs_table_data, self.plans[1].timetable_data.gtfs_table_data], axis=0)
    ...
    @staticmethod
    def has_records_with_vehicle_number(row, result_df):
        return result_df[
