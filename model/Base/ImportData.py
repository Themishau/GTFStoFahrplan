...
class ImportData(QObject):
    ...
    def read_gtfs_data(self, import_settings_dto: ImportSettingsDto):
        ...
        with zipfile.ZipFile(import_settings_dto.input_path) as zf:
            ...
            if self._pkl_loaded:
                logger.debug('pickle data detected')
    ...
    def load_pickleData(self, zf, df_gtfs_data, import_settings_dto):
        ...
        try:
            ...
            logger.debug('no feed info header')
    ...
    def read_file_from_zip(self, zip_file, filename, start_line=0):
        ...
        try:
            ...
            logger.debug(f'Error reading {filename}: {str(e)}')
            return []
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
            ...
            for result in concurrent.futures.as_completed(processes):
                ...
                try:
                    ...
                    logger.debug(f"Error in dict creation for {name}: {str(e)}")
                    return None

        logger.debug(f"raw_dict_data creation: {raw_dict_data.keys()}")
        self.progress_Update.emit(
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
            ...
            for result in concurrent.futures.as_completed(processes):
                ...
                try:
                    ...
                    logger.debug(f"Error in DataFrame creation: {str(e)} {df.name.value if df is not None else 'unknown df'}")
                    return None
        self.progress_Update.emit(
            self.progress.set_progress(80, ProcessType.import_data, "optimizing data for searching trips"))
        logger.debug(f"df_collection creation: {df_collection.keys()}")
    ...
    def create_df_routes(self, raw_data):
        logger.debug("convert to df: create_df_routes")
        ...
        try:
            ...
            logger.debug("can not convert df_routes: stop_id into int ")
        except ValueError:
            logger.debug("can not convert df_routes")
        except AttributeError:
            logger.debug("can not convert df_routes Attribute")
        self.missing_columns_in_df(df_routes, DfRouteColumnEnum)
        logger.debug("convert to df: df_routes finished")

        return df_routes

    def create_df_trips(self, raw_data):
        logger.debug("convert to df: create_df_trips")
    ...
    def create_df_stop_times(self, raw_data):
        ...
        try:
            ...
            logger.debug("can not convert df_stoptimes")
        except OverflowError:
            logger.debug("can not convert df_stoptimes")
        except ValueError:
            logger.debug("can not convert df_stoptimes")
        except AttributeError:
            logger.debug("can not convert df_stoptimes Attribute")
        self.missing_columns_in_df(df_stoptimes, DfStopTimesColumnEnum)
        logger.debug("convert to df: create_df_stop_times finished")

        return df_stoptimes

    def create_df_stops(self, raw_data):
        logger.debug("convert to df: create_df_stops")
        ...
        try:
            ...
            logger.debug("can not convert df_Stops: stop_id into int ")
        except ValueError:
            logger.debug("can not convert df_Stops")
        except AttributeError:
            logger.debug("can not convert df_Stops Attribute")
        self.missing_columns_in_df(df_stops, DfStopColumnEnum)
        logger.debug("convert to df: create_df_stops finished")
        return df_stops

    def create_df_week(self, raw_data):
        logger.debug("convert to df: create_df_week")
        ...
        try:
            ...
            logger.debug("can not convert df_week")

        self.missing_columns_in_df(df_week, DfCalendarweekColumnEnum)
        logger.debug("convert to df: df_week finished")

        return df_week

    def create_df_dates(self, raw_data):
        logger.debug("convert to df: create_df_dates")
        ...
        try:
            ...
            logger.debug("can not convert df_dateS")
        logger.debug("convert to df: create_df_dates finished")
    ...
    def create_df_agency(self, raw_data):
        logger.debug("convert to df: create_df_agency")
    ...
    def create_df_feed(self, raw_data):
        logger.debug("convert to df: create_df_feed")
    ...
    def load_raw_gtfs_data(self, import_settings_dto):
        ...
        try:
            ...
            logger.debug(f'Error processing GTFS files: {str(e)}')
            return None
        except:
            logger.debug('no feed info data')

        logger.debug(f"raw_data keys: {raw_data.keys()}")
        self.progress_Update.emit(
