...
class PlannerCacheTests(unittest.TestCase):
    ...
    def test_date_plan_matches_legacy_source_and_exports(self):
        ...
        self.assertIs(cached.plan.gtfs_data, self.source)
        result = cached.plan.timetable_data.timetable
        pd.testing.assert_frame_equal(result, expected.plan.timetable_data.timetable)
        ...
        PlanExporter().export_timetable(settings, cached.plan.timetable_data)
    ...
    def test_weekday_plan(self):
        ...
        strategy.create_timetable()
        self.assertFalse(strategy.plan.timetable_data.timetable.empty)
    ...
    def test_calendar_dates_only_plan(self):
        ...
        strategy.create_timetable()
        self.assertEqual(strategy.plan.timetable_data.timetable.iloc[:, 0].tolist(),
                         ["06:30", "06:40"])
    ...
    def test_qthread_import_and_reopen_without_zip(self):
        from viewmodel.application_view_model import ApplicationViewModel
